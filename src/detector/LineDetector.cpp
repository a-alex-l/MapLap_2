...
static cv::Point get_point(double coord, double side) {
    if (coord < 1)      return cv::Point(coord * side, 0);
    else if (coord < 2) return cv::Point( side, (coord - 1) * side);
    else if (coord < 3) return cv::Point((3 - coord) * side, side);
    else                return cv::Point(0, (4 - coord) * side);
}

static std::vector<double> get_coordinates(int x1, int x2,
                                           int y1, int y2,
                                           double side) { //return <[0,4), [0,4)>
    std::vector<double> answer;
    answer.reserve(2);
    if (y2 - y1 != 0) {
        double t = double(y2) / double(y2 - y1);
        if (0 <= x1 * t + (1 - t) * x2 && x1 * t + (1 - t) * x2 < side)
            answer.push_back((x1 * t + (1 - t) * x2) / side);
    }
    if (x2 - x1 != 0) {
        double t = double(x2 - side) / double(x2 - x1);
        if (0 <= y1 * t + (1 - t) * y2 && y1 * t + (1 - t) * y2 < side)
            answer.push_back((y1 * t + (1 - t) * y2) / side + 1);
    }
    if (y2 - y1 != 0) {
        double t = double(y2 - side) / double(y2 - y1);
        if (0 <= x1 * t + (1 - t) * x2 && x1 * t + (1 - t) * x2 < side)
            answer.push_back(-(x1 * t + (1 - t) * x2) / side + 3);
    }
    if (x2 - x1 != 0) {
        double t = double(x2) / double(x2 - x1);
        if (0 <= y1 * t + (1 - t) * y2 && y1 * t + (1 - t) * y2 < side)
            answer.push_back(-(y1 * t + (1 - t) * y2) / side + 4);
    }
    return answer;
}
...
void LineDetector::find_lines_parameters() {
    ...
    for (int step = 0; step < black_points.size() * 100; step++) {
        ...
        for (double t : check)
            good += input_contour(
                    t * black_points[i].first + (1 - t) * black_points[j].first,
                    t * black_points[i].second + (1 - t) * black_points[j].second);
        if (good > 5) {
...
void LineDetector::show() {
    ...
    for (auto &i : lines_parameters) {
        for (auto &j : i.second) {
            ...
            img.at<uchar>(i.first * 200 + 50, j.first * 200 + 50) =
                    std::max(int(img.at<uchar>(i.first * 200 + 50, j.first * 200 + 50))
                        - j.second * 5, 0);
        }
    }
